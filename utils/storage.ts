import { miniDebug, miniDebugTrace } from './debugLog';
...
export function mergeMealieCredentialsFromLocalIfNeeded(
    ...
    miniDebug('storage', 'sync missing credentials; checking storage.local mirror', {
        hasSyncServer: Boolean(syncData.mealieServer),
        hasSyncToken: Boolean(syncData.mealieApiToken),
    });
    ...
    chrome.storage.local.get([...MEALIE_CREDENTIAL_LOCAL_KEYS], (local: Partial<StorageData>) => {
        ...
        if (
            ...
            miniDebug('storage', 'Backfilling chrome.storage.sync from local credential mirror');
            void miniDebugTrace('credential-merge', 'sync backfilled from storage.local', {
                hadPartialSyncServer: Boolean(syncData.mealieServer),
                hadPartialSyncToken: Boolean(syncData.mealieApiToken),
            });
...
async function runStorageCheckAfterMerge(checkId: number, data: StorageData): Promise<void> {
    ...
    if (!mealieServer || !mealieApiToken) {
        miniDebug('badge-menu', 'No Mealie credentials after merge — clearing badge/menus');
        showBadge('❌');
        removeContextMenu();
    ...
    if (url && isRestrictedUrl(url)) {
        miniDebug('badge-menu', 'Active tab is restricted — menus hidden', {
            url: sanitizeUrl(url),
        });
        disableAllMenus();
        recordBadgeMenuRefresh({
